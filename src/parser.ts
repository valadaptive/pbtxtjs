...
class Tokenizer {
  ...
  public consumeByteString(): Uint8Array {
    const parts: Uint8Array[] = [this._consumeSingleByteString()];
  ...
  private _consumeSingleByteString(): Uint8Array {
...
function cUnescape(source: string): Uint8Array {
  const result: number[] = [];
  let i = 0;
  while (i < source.length) {
    if (source[i] === '\\' && i + 1 < source.length) {
      ...
      if (char >= '0' && char <= '7') {
        ...
        switch (char) {
          case 'a': result.push(0x07); break;
          case 'b': result.push(0x08); break;
          case 'f': result.push(0x0c); break;
          case 'n': result.push(0x0a); break;
          case 'r': result.push(0x0d); break;
          case 't': result.push(0x09); break;
          case 'v': result.push(0x0b); break;
